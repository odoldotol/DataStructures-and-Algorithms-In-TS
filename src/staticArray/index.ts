  ...
  extends Array<T>
{
  ...
  가장 안전하고 직관적인 방법은 CAPACITY 와 _length 를 새로 정의하는 것일 것이다. 기존 length 는 유명무실해지지만 이해하기 쉽다.
   ...
   * @StaticArray Throws an error if the new length exceeds the capacity.
   */
  ...
  pop, shilft => delete operator 를 이용한다. StaticArray 는 delete 를 허용하지 않는다.
  splice => redefine 을 이용한다. StaticArray 에서는 redefine 을 허용하지 않는다. */
  ...
  /**
   * @StaticArray Throws an error if array is empty.
   */
